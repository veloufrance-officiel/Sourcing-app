...
export function MissionForm({ clients }: { clients: { id: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} className="mt-6 space-y-5">
      <div className="rounded-xl border border-line bg-white p-4 shadow-sm">
        ...
        <fieldset className="mt-4">
          ...
          <div className="mt-2 flex gap-4">
            ...
            <label className="flex items-center gap-2 text-sm text-ink">
              <input type="radio" name="source" value="direct" required className="h-4 w-4 accent-[#2563eb]" />
              Direct
            </label>
          </div>
        </fieldset>
      ...
      <div className="rounded-xl border border-line bg-white p-4 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-wider text-slate">Détails de la mission</p>
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="TJM (€)" name="daily_rate" type="number" placeholder="350" mono />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-line bg-white p-4 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <label htmlFor="brief_raw" className="text-xs font-semibold uppercase tracking-wider text-slate">
      ...
      <button
        type="submit"
        disabled={pending}
        className="rounded-lg bg-ink px-5 py-2.5 text-sm font-medium text-paper hover:bg-ink/90 disabled:opacity-50"
      >
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`mt-1 w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink outline-none focus:border-signal focus:ring-2 focus:ring-signal/20 ${mono ? 'font-mono' : ''}`}
      />
    </div>
  )
}
